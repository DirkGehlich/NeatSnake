...
public class Population {
	public ArrayList<Player> pop = new ArrayList<Player>();
	Player bestPlayer;// the best ever player
	double bestFitness = 0;// the score of the best ever player
	// int species = 0;
	int gen;
	...
	Population(int size, int numInputs, int numOutputs) {
		...
		for (int i = 0; i < size; i++) {
			pop.add(new Player(numInputs, numOutputs));
	...
	void updateAlive() {
		...
		for (int i = 0; i < pop.size(); i++) {
			if (!pop.get(i).dead) {
				...
				if (!Parameters.showNothing && (!Parameters.showBest || i == 0)) {
					pop.get(i).show();
				}
			}
		}
	...
	void setBestPlayer() {
		...
		// if best this gen is better than the global best score then set the global
		// best as the best this gen

		if (tempBest.fitness > bestFitness) {
			genPlayers.add(tempBest.cloneForReplay());
			System.out.println("old best: " + bestFitness);
			System.out.println("new best: " + tempBest.fitness);
	...
	// ------------------------------------------------------------------------------------------------------------------------------------------------
	// this function is called when all the players in the population are dead and a
	// new generation needs to be made
	...
	public void naturalSelection() {
		...
		System.out.println("Number of mutations " + innovationHistory.size());
		...
		for (int j = 0; j < species.size(); j++) {// for each species
			...
			for (int i = 0; i < species.get(j).players.size(); i++) {
				System.out.println("player " + i + "\tfitness: " + species.get(j).players.get(i).fitness);
			}
			System.out.println();
			children.add(species.get(j).champ.cloneForReplay());// add champion without any mutation

			int NoOfChildren = (int) Math.floor(species.get(j).averageFitness / averageSum * pop.size()) - 1;// the
																												// number
																												// of
			// children this
			// species is
		...
		while (children.size() < pop.size()) {// if not enough babies (due to flooring the number of children to get a
												// whole int)
	...
	// ------------------------------------------------------------------------------------------------------------------------------------------
	// seperate population into species based on how similar they are to the leaders
	// of each species in the previous gen
	void speciate() {
		...
		for (int i = 0; i < pop.size(); i++) {// for each player
			...
			for (Species s : species) {// for each species
				if (s.sameSpecies(pop.get(i).brain)) {// if the player is similar enough to be considered in the same
														// species
			...
			if (!speciesFound) {// if no species was similar enough then add a new species with this as its
								// champion
	...
	// ------------------------------------------------------------------------------------------------------------------------------------------
	// if a species sucks so much that it wont even be allocated 1 child for the
	// next generation then kill it now
	void killBadSpecies() {
